## 아래의 코드(tool_gui_v04.py)에 대한 보완사항
## - 기존에는 차량과 차량의 외부를 나누는 경계를 텍스트로 입력받도록하는 UI가 없음 
## 차량과 차량의 외부를 나누는 경계를 텍스트로 입력받도록 UI를 수정해야함 
...
DEFAULT_TOOL2_SCRIPT = "../backend/parse_attack_graph_v10py"
DEFAULT_OUT_JSON = "../out/attack_graph.json"
DEFAULT_ASSET_MAP = "../backend/threat_library/asset_to_threats.json"
...
class Tool2GUI(tk.Tk):
    def __init__(self):
        ...
        # 차량(내부) vs 차량 외부를 나누는 Trust Boundary/Border(BorderBoundary) 이름
    ...
    def _build_ui(self):
        ...
        ttk.Label(row, text="차량/외부 경계명(Trust Boundary/Border)").grid(row=3, column=0, sticky="w", padx=8, pady=6)
        e = ttk.Entry(row, textvariable=self.var_boundary)
        e.grid(row=3, column=1, sticky="ew", padx=8, pady=6)
    ...
    def _validate_inputs(self) -> bool:
        ...
        # Remote/Adjacent 모드에서는 Trust Boundary/Border 이름이 필요 (Entry point 판별에 사용)
        if mode == "Remote/Adjacent" and not boundary:
            messagebox.showerror(
                "Missing",
                "Remote/Adjacent 모드에서는 '차량/외부 경계명(Trust Boundary/Border)'을 입력해주세요.",
            )
            return False

        # Remote/Adjacent 모드에서는 boundary 입력이 필수(backend가 --boundary를 요구)
        if mode == "Remote/Adjacent" and not boundary:
            messagebox.showerror("Missing", "차량/외부 경계명(Trust Boundary/Border)을 입력해주세요.")
            return False
    ...
    def _run_tool2_subprocess(self):
        ...
        # Remote/Adjacent 모드에서만 boundary 사용 (Local/Physical은 보통 불필요)
        if boundary and mode == "Remote/Adjacent":
