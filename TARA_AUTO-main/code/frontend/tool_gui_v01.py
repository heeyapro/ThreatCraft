...
DEFAULT_TOOL2_SCRIPT = "tool2_attack_graph.py"
...
class Tool2GUI(tk.Tk):
    def __init__(self):
        ...
        self.var_asset_map = tk.StringVar(value=DEFAULT_ASSET_MAP)
        self.var_threat_map = tk.StringVar(value=DEFAULT_THREAT_MAP)
        self.var_out_json = tk.StringVar(value=DEFAULT_OUT_JSON)
        self.var_max_depth = tk.StringVar(value="30")
        self.var_tool2_script = tk.StringVar(value=DEFAULT_TOOL2_SCRIPT)
    ...
    def _build_ui(self):
        ...
        # --- Tool-2 script path ---
        row = ttk.LabelFrame(root, text="Tool-2 Script")
        row.pack(fill="x", **pad)

        ttk.Label(row, text="tool2_attack_graph.py 경로").grid(row=0, column=0, sticky="w", padx=8, pady=6)
        e = ttk.Entry(row, textvariable=self.var_tool2_script)
        e.grid(row=0, column=1, sticky="ew", padx=8, pady=6)
        ttk.Button(row, text="Browse", command=self._browse_tool2_script).grid(row=0, column=2, padx=8, pady=6)
        ...
        ttk.Label(row2, text="asset_to_threats.json").grid(row=0, column=0, sticky="w", padx=8, pady=6)
        e = ttk.Entry(row2, textvariable=self.var_asset_map)
        e.grid(row=0, column=1, sticky="ew", padx=8, pady=6)
        ttk.Button(row2, text="Browse", command=self._browse_asset_map).grid(row=0, column=2, padx=8, pady=6)

        ttk.Label(row2, text="threat_to_tactic.json").grid(row=1, column=0, sticky="w", padx=8, pady=6)
        e = ttk.Entry(row2, textvariable=self.var_threat_map)
        e.grid(row=1, column=1, sticky="ew", padx=8, pady=6)
        ttk.Button(row2, text="Browse", command=self._browse_threat_map).grid(row=1, column=2, padx=8, pady=6)
