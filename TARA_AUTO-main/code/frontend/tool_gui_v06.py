...
class Tool2GUI(tk.Tk):
    def __init__(self):
        ...
        self.var_merged_graph_prefix = tk.StringVar(value=str((self.base_dir / DEFAULT_MERGED_GRAPH_PREFIX).resolve()))
        self.var_merged_graph_format = tk.StringVar(value="png")
        ...
        # Toggles
        self.var_render_merged_graph = tk.BooleanVar(value=True)
        self.var_render_attack_tree = tk.BooleanVar(value=True)
    ...
    def _build_ui(self):
        ...
        # 2) Merged Attack Graph image (prefix + format)
        ttk.Label(lf_out, text="Attack Graph Image (--merged-graph-out prefix)").grid(row=1, column=0, sticky="w", padx=8, pady=6)
        ttk.Entry(lf_out, textvariable=self.var_merged_graph_prefix).grid(row=1, column=1, sticky="ew", padx=8, pady=6)
        ttk.Button(lf_out, text="Browse", command=self._browse_merged_graph_prefix).grid(row=1, column=2, padx=8, pady=6)

        ttk.Label(lf_out, text="Graph Format").grid(row=2, column=0, sticky="w", padx=8, pady=6)
        ttk.Combobox(
            lf_out,
            textvariable=self.var_merged_graph_format,
            values=["png", "svg", "pdf"],
            state="readonly",
            width=10,
        ...
        # 3) Attack Tree PNG
        ttk.Label(lf_out, text="Attack Tree Image (--attack-tree-png)").grid(row=3, column=0, sticky="w", padx=8, pady=6)
        ttk.Entry(lf_out, textvariable=self.var_attack_tree_png).grid(row=3, column=1, sticky="ew", padx=8, pady=6)
        ttk.Button(lf_out, text="Browse", command=self._browse_attack_tree_png).grid(row=3, column=2, padx=8, pady=6)
        ...
        ttk.Checkbutton(
            lf_opt,
            text="Render Attack Graph Image (--render-merged-graph)",
            variable=self.var_render_merged_graph,
        ).grid(row=1, column=0, columnspan=2, sticky="w", padx=8, pady=4)

        ttk.Checkbutton(
            lf_opt,
            text="Render Attack Tree (--render-attack-tree)",
            variable=self.var_render_attack_tree,
    ...
    def _browse_merged_graph_prefix(self):
        # prefix라서 "저장 파일"이 아니라 폴더+이름까지 지정하게 하는 게 편함
        path = filedialog.asksaveasfilename(
            title="Set merged graph output prefix (no extension)",
            defaultextension="",
            filetypes=[("No extension (prefix)", "*.*")],
            initialfile=Path(self.var_merged_graph_prefix.get() or DEFAULT_MERGED_GRAPH_PREFIX).name,
        )
        if path:
            # 사용자가 .png 같은 확장자를 붙여도 prefix로 쓰기 위해 제거
            p = Path(path)
            if p.suffix:
                p = p.with_suffix("")
            self.var_merged_graph_prefix.set(str(p))
    ...
    def _validate_inputs(self) -> bool:
        ...
        if self.var_render_merged_graph.get():
            prefix = self.var_merged_graph_prefix.get().strip()
            if not prefix:
                messagebox.showerror("Missing", "Attack Graph Image prefix(--merged-graph-out)를 지정해주세요.")
                return False

        if self.var_render_attack_tree.get():
    ...
    def _run_tool2_subprocess(self):
        ...
        render_merged = self.var_render_merged_graph.get()
        merged_prefix = self.var_merged_graph_prefix.get().strip()
        merged_fmt = self.var_merged_graph_format.get().strip()

        render_tree = self.var_render_attack_tree.get()
        ...
        if render_merged:
            ...
            cmd.append("--no-render-merged-graph")
        ...
        if render_tree:
            ...
            cmd.append("--no-render-attack-tree")
        ...
        try:
            ...
            def done():
                ...
                if proc.returncode == 0:
                    self._log("\n[OK] Done.\n")
                    # 사용자에게 결과 파일 경로를 한 번에 보여주기
                    msg_lines = [f"Attack Graph JSON:\n{out_json}"]
                    if render_merged:
                        msg_lines.append(f"\nAttack Graph Image:\n{merged_prefix}.{merged_fmt}")
                    if render_tree:
                        msg_lines.append(f"\nAttack Tree Image:\n{attack_tree_png}")
