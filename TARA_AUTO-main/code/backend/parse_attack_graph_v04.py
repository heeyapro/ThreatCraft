...
✅ 이번 반영 사항(요청한 1번만 반영):
  - Out phase 위협은 "Target 자산에서만" 선택 가능
...
def dfs_backward(
    st: GraphBuildState,
    target_guid: str,  # ✅ 추가: Out은 이 target_guid에서만 허용
    cur_guid: str,
    ...
    ✅ 반영: Out phase는 Target 자산(target_guid)에서만 선택 가능
    """
    ...
    for th in cands:
        ...
        for flow in incoming_flows:
            ...
            dfs_backward(
                st=st,
                target_guid=target_guid,  # ✅ 재귀에도 전달
                cur_guid=src_guid,
...
def build_attack_graph_remote_adjacent(
    ...
    for tgt_guid in target_guids:
        ...
        for out_th in out_cands:
            ...
            dfs_backward(
                st=st,
                target_guid=tgt_guid,  # ✅ Out은 이 target 인스턴스에서만 허용
                cur_guid=tgt_guid,
