## 아래 코드(parse_attack_graph_v12.py)에 대한 보완사항. 
## - 기존에는 Graphviz 라이브러리를 활용하여 Dot 파일을 만들 때 다이어그램 내 노드의 크기가 내부 글의 길이에 따라 커졌음 (merged_attack_graph_local_physical.png 참고)
## - 노드의 크기를 전부 하나로 통일시키도록 수정
...
python parse_attack_graph_v13.py --tm7 "../in/Automotive DFD_B.tm7" --type "Local/Physical" --target "Door" --boundary "External Vehicle Boundary"  --asset-map "threat_library/asset_to_threats.json" --threat-map "threat_library/threat_to_tactic.json" --out "../out/attack_graph.json" --graph-out "../out/merged_attack_graph"
