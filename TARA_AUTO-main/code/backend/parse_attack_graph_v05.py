...
✅ 반영된 수정사항(2개)
  1) Out phase 위협은 "Target 자산에서만" 선택 가능
     (즉, Gateway/BCM 등이 Out 위협을 가지고 있어도 해당 경로에서는 Out으로 선택되지 않음)
  2) 경로 내에서 Target(예: Door)의 Out 노드가 중복으로 들어가는 버그 제거
     - build_attack_graph_remote_adjacent()에서 start_node_id로 이미 Target Out 노드를 path에 넣었으므로,
       dfs_backward() 첫 진입 시(Target에서) Target 위협 노드를 다시 append 하지 않고 incoming flow만 따라가게 함
"""
...
def dfs_backward(
    ...
    - Out phase는 Target 자산(target_guid)에서만 선택 가능
    - (FIX) Target Out 노드 중복 방지
    """
    ...
    # ✅ [FIX] Target(Door) Out 노드가 이미 path에 들어있으면,
    #         Target의 위협을 다시 추가하지 말고 incoming flow만 따라 올라간다.
        ...
        if last_node.get("asset_guid") == target_guid and last_node.get("phase") == "Out":
            ...
            for flow in incoming_flows:
                ...
                dfs_backward(
                    ...
                    cur_guid=src_guid,
                    allowed_max_phase=allowed_max_phase,  # 시작이 Out이면 보통 Out(2)로 들어옴
                    visited_assets=visited_assets,
                    path_node_ids=path_node_ids,          # ✅ Target Out 노드는 유지
                ...
                new_upstream_nodes = [
                    nid for nid in (after_nodes - before_nodes) if nid.startswith(src_guid + "::")
                ]
                # ✅ 새로 생긴 상류 노드들을 "현재 Target Out 노드"로 연결
                for up_id in new_upstream_nodes:
                    graph_edges.add((up_id, last_id, flow.guid))

                visited_assets.remove(src_guid)

            return  # ✅ Target 처리 끝 (중복 방지)
...
def build_attack_graph_remote_adjacent(
    ...
    for tgt_guid in target_guids:
        ...
        for out_th in out_cands:
            ...
            dfs_backward(
                ...
                visited_assets=visited_assets,
                path_node_ids=[start_node_id],  # ✅ Target Out 노드를 이미 넣고 시작
                graph_nodes=graph_nodes,
                graph_edges=graph_edges,
                all_paths=all_paths,
            )
    ...
    return {
        ...
        "path_fix": "Target Out node duplication is prevented (Target Out is added exactly once per path).",
    }
