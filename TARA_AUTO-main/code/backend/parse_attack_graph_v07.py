## 아래 코드에 대한 보완사항.
## - 기존에는 공격의 entry point에 해당하는 노드 1개 이상 -> In 1개인 노드 -> Through 1개 이상 노드 -> Out 1개 노드
## - 이제는 공격의 entry point에 해당하는 노드 1개 -> In 1개인 노드 -> Through 1개 이상 노드 -> Out 1개 노드
...
반영된 정책:
  ...
  3) In을 만나면 Entry 노드를 정확히 1개 붙이고 종료(저장) (boundary 기반)
"""
...
def parse_tm7(tm7_path: Path) -> Tuple[Dict[str, DFDNode], List[DFDFlow], List[dict]]:
    ...
    # Trust Boundary / Border (TM7에서 BorderBoundary로 나오는 경우)
...
def add_single_entry_before_in(
    ...
    """
    ✅ 변경: Entry는 정확히 1개만 허용.
    - In 노드로 들어오는 incoming flow 중
      entry_side에 위치한 src 노드 하나를 Entry로 선택해 path에 추가하고,
      그 시점에서 경로를 저장한다(더 이상 Entry를 추가하지 않음).

    여러 incoming 분기(여러 src)가 있으면, 각각이 "서로 다른 Entry=1" 경로 후보가 된다.
    """
    ...
    for flow in incoming_flows:
        ...
        # boundary 반대편(entry_side)에 있는 노드만 Entry 후보
        ...
        # ✅ 이 시점에서 Entry는 정확히 1개이므로 유효성 검사 후 저장
...
def dfs_backward(
    ...
    Through는 1개 이상, In은 정확히 1개가 되도록:
      - In을 선택하는 순간 더 이상 Threat 선택을 하지 않고 Entry(정확히 1개)를 붙인 뒤 저장.
    """
    ...
    # Target Out 중복 방지: 이미 path 마지막이 target Out이면 incoming만 탄다.
    ...
    for th in cands:
        ...
        # ✅ In 선택 시: Entry(정확히 1개)만 추가하고 저장 후 종료
        if th.phase == "In":
...
def build_attack_graph_remote_adjacent(
    ...
    # path는 [Out ... Entry] (역방향 append) 이므로, Entry -> ... -> Out으로 뒤집어서 저장
    ...
    return {
        ...
        "path_fix": "Target Out duplication prevented; In triggers exactly-one Entry selection.",
    }
