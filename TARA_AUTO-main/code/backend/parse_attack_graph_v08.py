## 아래 코드에 대한 보완사항.
## - 기존에는 Remote/Adjacent 모드에 대한 공격 경로 식별만 진행하였음
## - 이제는 Local/Physical 모드에 대한 공격 경로도 식별해야 함
## - Local/Physical 모드에 대한 공격 경로
##   : Through 1개 이상 노드 -> Out 1개 노드 형태의 공격 경로
...
입력:
  ...
  4) boundary_name: Entry point를 나누는 Trust Boundary/Border 이름(문자열)
     - Remote/Adjacent 에서만 의미 있음 (Local/Physical은 미사용)
...
공통 정책:
  ...
  2) 경로 내에서 Target의 Out 노드 중복 방지
  3) Remote/Adjacent에서만: In을 만나면 Entry 노드를 정확히 1개 붙이고 저장 (boundary 기반)
"""
...
python parse_attack_graph_v08.py --tm7 "Automotive DFD_B.tm7" --type "Remote/Adjacent" --target "Door" --boundary "External Vehicle Boundary" --asset-map "asset_to_threats.json" --threat-map "threat_to_tactic.json" --out "attack_graph.json"
python parse_attack_graph_v08.py --tm7 "Automotive DFD_B.tm7" --type "Local/Physical" --target "Door" --boundary "External Vehicle Boundary" --asset-map "asset_to_threats.json" --threat-map "threat_to_tactic.json" --out "attack_graph.json"
