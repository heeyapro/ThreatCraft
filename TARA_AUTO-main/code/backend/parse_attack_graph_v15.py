...
# ✅ attack_vector_feasability.json 로드
...
def parse_tm7(tm7_path: Path) -> Tuple[Dict[str, DFDNode], List[DFDFlow], List[dict]]:
    ...
    for el in root.iter():
        ...
        height = safe_float(get_child_text(el, "Height")) or 0.0
        boundaries.append(
            {"guid": guid, "name": name, "left": left, "top": top, "width": width, "height": height}
        )
...
def path_satisfies_dependencies(
    ...
    # 경로 내 flow (source_asset_name, dest_asset_name)
    ...
    for rule in dependencies:
        ...
        # -------------------
        # PRE_THREAT_ENABLES_POST_THREAT
        # -------------------
...
def build_attack_graph_remote_adjacent(
    ...
    if not valid_paths:
        return {
            "ok": False,
            "reason": (
                f"Phase 조건을 만족하는 경로는 있었지만, dependency.json 강제 규칙을 모두 만족하는 경로가 없습니다."
            ),
            "target_asset_name": target_asset_name,
            "boundary_name": boundary_name,
            "tm7_path": str(tm7_path),
...
# -----------------------------
# CLI  (CHANGED: 4 modes + attack_vector map + dependency map)
# -----------------------------
def main():
