## 아래 코드(parse_attack_graph_v11.py)에 대한 보완사항. 
## - 기존에는 JSON 형식의  파일에 공격 그래프를 저장하고 출력해주지는 않음
## - DFD 상의 공격 그래프를 Graphviz를 통하여 DFD 상의 전체 병합된 공격 그래프 출력 (In Phase 만 적용될 수 있는 DFD 노드는 초록색, Through Phase 만 적용되거나 In Phase, Through Phase 둘다 적용될 수 있는 DFD 노드는 주황색 (In Phase, Through Phase 둘다 적용될 수 있는 DFD 노드는 In/Through로 작게 DFD 요소 밑에 써주기), Out Phase만 적용될 수 있는 DFD 노드는 빨간색) (적용 될수 있는 위협 Tactic 명과 식별자는 표기하지 않음.)
## - 예를 들어, 
## 식별된 Remote/Adjacent 공격 경로가 smart phone(Entry Point) -> TCU (In 적용 가능) -> BCM (Through 적용 가능) -> Door (Out 적용 가능),
## 이랑 smart phone -> dummy(In 적용 가능) -> TCU (Through 적용 가능) -> BCM (Through 적용 가능) -> Door (Out 적용 가능) 일때
## 공격 그래프는 smart phone -> dummy
...
# Graphviz rendering (see test_graphviz.py style)
...
def is_valid_attack_path_local(path_node_ids: List[str], graph_nodes: Dict[str, dict]) -> bool:
    ...
    return (cnt["Through"] >= 1 and cnt["Out"] == 1)
...
def build_merged_dfd_subgraph(
    *,
    ...
    - Nodes: DFD elements (asset GUIDs) that appear in any path.
    - Edges: DFD connectors (flow GUIDs) used by any path edge.

    This is *not* a single path; it's a merged subgraph on the original DFD.
    """
    # 1) DFD nodes used
    ...
    # 3) Start / end (DFD-level) for convenience
...
def dfs_backward_local_physical(
    ...
    # ✅ Local/Physical에서는 Through만 의미 있음 (In은 스킵)
    cands = threat_candidates_for_asset(
        asset_name=dfd_node.name,
        asset_to_threats=st.asset_to_threats,
        tactic_to_phase=st.tactic_to_phase,
    ...
    for th in cands:
        ...
        # Local/Physical은 In을 사용하지 않음(정의상)
        if th.phase == "In":
            continue
        ...
        # ✅ Through를 1개 이상 확보한 시점부터는(=현재까지 유효하면) 저장 가능
        if is_valid_attack_path_local(path_node_ids, graph_nodes):
            all_paths.append(list(path_node_ids))
...
def build_attack_graph_remote_adjacent(
    ...
    # ✅ remove fully identical paths (same node_id sequence)
...
def build_attack_graph_local_physical(
    ...
    if not valid_paths_raw:
        return {
            ...
            "reason": (
                f"목표 자산 '{target_asset_name}' 기준으로 탐색된 경로 중 "
                f"'Through>=1 & Out==1' 조건을 만족하는 경로가 없습니다."
            ),
            "target_asset_name": target_asset_name,
            "nodes": [],
            "edges": [],
    ...
    # Local도 동일하게 보기 좋게 upstream->...->target(Out) 방향으로 뒤집어 저장
    valid_paths = [list(reversed(p)) for p in valid_paths_raw]
    ...
    return {
        ...
        "paths": valid_paths,
        "merged_dfd_subgraph": merged_dfd_subgraph,
        "phase_order": ["Through", "Out"],
        "path_constraint": "Through>=1 & Out==1",
        "out_phase_policy": "Out phase is allowed only on the target asset in this search.",
        "note": "Local/Physical mode ignores Entry/In and searches only Through->Out paths.",
    }
...
def _shape_from_stencil(stencil_type: str) -> str:
    """Map TM7 stencil types to Graphviz node shapes."""
    if stencil_type == "StencilParallelLines":
        return "cylinder"
    if stencil_type == "StencilEllipse":
        return "circle"
    # default (StencilRectangle etc.)
    return "box"


def _phase_style(phases: Set[str]) -> Tuple[str, str]:
    """Return (fillcolor, phase_note) for a DFD node.

    Colour rules (requested):
      - In only -> green
      - Through only OR In+Through -> orange
      - Out only -> red
      - Entry only -> lightgrey (not specified, but helps show start in Remote/Adjacent)
    """
    p = set(phases)
    p.discard(None)
    # Out (target) should be red
    if p == {"Out"}:
        return "red", "Out"
    # In only
    if p == {"In"}:
...
def render_merged_attack_graph_graphviz(
    ...
    """Render merged_dfd_subgraph to a Graphviz diagram.

    - Does NOT print threat IDs / tactic names.
    - Colours nodes by the *applicable phase(s)* observed in the merged attack subgraph.
    """
    ...
    # asset_guid -> set(phases) gathered from threat-graph nodes that survived pruning
    ...
    for n in dfd_nodes:
        ...
        # HTML label: Bold name + small phase note
        if note and note not in ("Through", "In", "Out"):
            # In/Through or combined
            label = f"<<B>{name}</B><br/><FONT POINT-SIZE='9'>{note}</FONT>>"
        elif note:
            label = f"<<B>{name}</B><br/><FONT POINT-SIZE='9'>{note}</FONT>>"
        else:
            label = f"<<B>{name}</B>>"
...
def main():
    ...
    ap.add_argument("--boundary", default="", help="(Remote/Adjacent 전용) Entry point를 나누는 Trust Boundary/Border 이름(BorderBoundary Name)")
...
python parse_attack_graph_v12.py \
    --tm7 "../in/Automotive DFD_B.tm7" \
    --type "Local/Physical" \
    --target "Door" \
    --boundary "External Vehicle Boundary" \
    --asset-map "threat_library/asset_to_threats.json" \
    --threat-map "threat_library/threat_to_tactic.json" \
    --out "../out/attack_graph.json" \
    --graph-out "../out/merged_attack_graph"
