...
def _calc_cal_label(impact_label: str, mode: str) -> str:
    # ISO/SAE 21434:2021 Annex E, Table E.1 example mapping
    ...
    for idx, path in enumerate(data.get("paths", []), start=1):
        ...
        for nid in path:
            ...
            row.append({
                ...
                "threat_name": n.get("threat_name", n.get("threat_id", "?")),
                "phase": n.get("phase", "?"),
                "tactic": n.get("tactic", "?"),
            })
        rendered.append(row)
...
def _format_path(path_rows: List[dict]) -> str:
    ...
    for step in path_rows:
        chunks.append(
            f"{_safe_text(step.get('asset_name'))} | {_safe_text(step.get('phase'))} | {_safe_text(step.get('threat_name'))}"
        )
...
def _format_path_fixed(path_rows: List[dict]) -> str:
    ...
    for step in path_rows:
        ...
        if step.get('phase')=="Out":
            ...
            color = "color:green;"
        chunks.append(
            f"{_safe_text(step.get('asset_name'))} (via <span style=\"{color}\"> {_safe_text(step.get('threat_name'))}</span>)"
        )
    return " &rarr; ".join(chunks)
...
def _build_asset_detail_text(asset_name: str, threats: List[dict]) -> str:
    ...
    seen = set() 
    for th in threats: 
        threat_name = (th.get("threat_name") or th.get("threat_id") or "-").strip() 
        if threat_name in seen: 
            continue
        seen.add(threat_name)
        lines.append(f"- {threat_name}")
    return "\n".join(lines)
    '''
...
def _build_cytoscape_dfd_model(tm7_path: Path, attack_graph_result: dict) -> dict:
    ...
    # asset_guid별 대표 phase 선택 (Out > Through > In > Entry)
...
## 경로 별 Risk 계산
## Path 별로 자산 별 Impact * Feasability 
...
def generate_html_report(
    ...
    threat_index = _load_threat_index(threat_cti_path)
    target_asset = _load_target_asset_record(asset_map_path, target_asset_name)
    impact_record = _load_impact_record(impact_map_path, target_asset_name)
    feasibility_index = _load_feasibility_index(attack_vector_path)
    ...
    ## 디버깅
    ## print(path_rows)
    data_with_risk = _read_json(attack_graph_with_risk_path)   ## risk 점수가 포함된 path  정보
    path_with_risk_rows =_build_path_descriptions_for_path_with_risk(data_with_risk) 
    
    cy_model = _build_cytoscape_dfd_model(Path(tm7_path), data)
    ...
    threat_rows_html = "".join(
        ...
        <tr>
            ...
            <td>{_safe_text(row['threat_name'])}</td>
    ...
    print(enumerate(path_rows, start=1))
    ...
    attack_graph_img_html = (
        ...
        f"<img class='graph-image' src='{_safe_text(os.path.basename(str(attack_graph_path)))}'>"
        "</div>"
...
<head>
    ...
    <style>
        ...
        .scenario-col-id {{ width: 10%; white-space: nowrap; }}
        .scenario-col-name {{ width: 22%; }}
        .scenario-col-tactic {{ width: 16%; }}
        .scenario-col-text {{ width: 52%; }}
        ul {{ margin-top: 6px; }}
        ...
        @media print {{
            ...
            .metric b {{ font-size: 9.5pt; }}
            th, td {{ padding: 5px 6px; }}
            .card {{ padding: 8px 10px; margin-top: 8px; }}
            .graph-image {{ width: 100%; max-width: 100%; height: auto; max-height: 95mm; }}
            .feasibility-section {{ page-break-before: always; break-before: page; }}
        }}
        .diagram-section {{
...
<body>
    <div class="container">
    <h1>Threat Analysis and Risk Assessment Report</h1>
    ...
    <div class=\"section\">
    	...
    	<table>
        	<thead>
            	<tr>
                	<th class="path-id-col">Path ID</th>
                	<th>Attack Path</th>
                	<th style=\"width: 50px;\">Risk Score</th>
            	</tr>
        	</thead>
        	<tbody>
            	{path_rows_html}
    	...
    	<div class=\"diagram-section\">
            <h3>DFD Attack Path Graph</h3>

            <div class=\"screen-only\">
                <div class=\"diagram-layout\">
                    <aside class=\"side-panel\">
                        <div class=\"small\">
                            The full input DFD is rendered below. Only the attack-graph-relevant DFD elements and
                            data flows are highlighted. Phase colours are:
                            <br><br>
            ...
            <div class=\"print-only\">
                <div class=\"card\">
                    <p class=\"small\">
                        Static image view for PDF export. The interactive Cytoscape view is shown only in the browser.
                    </p>
                    {attack_graph_img_html}
                </div>
            </div>
    ...
    <div class=\"card\">
        ...
        <p class=\"small\">This risk matrix is a pragmatic project rule for automated reporting. The CAL value is separately derived using the ISO/SAE 21434 Annex E example table.</p>
    </div>
    </div>
...
def dfs_backward_remote(
    ...
    for th in cands:
        ...
        if th.phase == "In":
            ...
            # In→In 체인: 이 In 이전에 또 다른 In 위협 자산으로 거슬러 탐색
            incoming_flows = st.incoming.get(cur_guid, [])
            for flow in incoming_flows:
                src_guid = flow.source_guid
                if src_guid in visited_assets:
...
def _render_report_preview_from_graph(result: dict, out_prefix: Path | str, font: str = "Malgun Gothic") -> Optional[Path]:
    ...
    return render_merged_attack_graph_graphviz(
        result=result,
        out_prefix=preview_prefix,
        fmt="png",
        font=font,
    )
...
def main():
    ...
    ap.add_argument("--impact-map", default="impact_map.json", help="impact_map.json 경로")
    ...
    ap.add_argument("--merged-graph-format", default="pdf", choices=["png", "svg", "pdf"], help="Merged graph 포맷 (기본: pdf, 보고서 미리보기용 PNG는 자동 생성)")
    ...
    ap.add_argument("--detection-report-pdf", default="", help="결과 보고서 PDF 출력 경로(비우면 detection-report와 같은 이름의 .pdf)")

    ## (03.17 메모) LLM API키를 받는 파라미터 추가
    ap.add_argument("--llm-api-key", default="", help="Google Gemini API Key")    
    ...
    if args.render_merged_graph:
        try:
            ...
            if args.merged_graph_format.lower() != "png":
                ...
                report_preview_graph = _render_report_preview_from_graph(
                    result=result,
                    out_prefix=preview_prefix,
                )
                if report_preview_graph:
                    print(f"[OK] rendered merged attack graph preview for report: {report_preview_graph}")
            else:
                report_preview_graph = rendered
        except Exception as e:
            print(f"[WARN] merged graphviz rendering failed: {e}", file=sys.stderr)

    ## 공격 그래프 식별 결과 json에서, 공격 경로 별로 Risk 점수 추가
    add_risk_to_paths(out_path,"../out/attack_graph_with_risk_temp.json", attack_vector_map,asset_map,threat_map,impact_map,"./threat_library/impact_feasability_map.json")
    ...
    attack_tree_path_for_report = Path(args.attack_tree_png) if args.render_attack_tree else None


    ## (03.17 메모) LLM에 쿼리 및 응답 조회 (필요에 따라 위치 조정, 현재파라미터는 API Key 하나만 있는데, 필요한 데이터들 있으면 함수 정의 가서 파라미터 추가)
    gemini_summary = _run_gemini_flash_lite_on_report(api_key=args.llm_api_key)
    ## (03.17) LLM 응답 디버깅용 
    '''
    ...
    if args.detection_report:
        ...
        generate_html_report(
            ...
            out_path=out_path,
            threat_cti_path=threat_map,
            asset_map_path=asset_map,
            attack_vector_path=attack_vector_map,
            impact_map_path=impact_map,
            attack_graph_with_risk_path = "../out/attack_graph_with_risk_temp.json",    # Risk값이 반영된 분석 보고서에 대한 경로
            tm7_path = tm7_path
            )
...
## 
def add_risk_to_paths(in_json_path : Path, out_json_path : Path, asset_feasability_json_path : Path, asset_to_threat_json : Path, threat_to_tactic_json : Path, impact_map_json : Path, impact_feasability_map : Path):
    ...
    for idx in range(len(result["paths"])):
        ...
        for target_node_id in p:
            ## target_node_id = 현재 노드 ID
            for node in result["nodes"]:
                if node["node_id"] == target_node_id:
                    ## Entry 노드는 Risk 계산 과정에서 고려하지 않음
                    if node["phase"] == "Entry":
                    ...
                    if not current_feasability:
                         ## 현재 노드에서의 fesability를 계산
                         ## attack_vector_feasability.json 에서 매칭되는 Threat 의 feasability 조회
                    ...
                    ##print(f"자산 : {node["asset_name"]}, UKC 단계: {phases}")
...
def _run_gemini_flash_lite_on_report(api_key: str) -> Optional[str]:
    api_key = (api_key or "").strip()
    if not api_key:
        print("[INFO] Gemini API key not provided. Skipping Gemini call.")
        return None
    ...
    try:
        ...
        print(f"[WARN] google-genai SDK import failed: {e}", file=sys.stderr)
        print("[WARN] Install with: pip install -U google-genai", file=sys.stderr)
        return None

    # (03.17 메모) LLM에 입력될 프롬프트
    prompt = f""" blah blah .... """
...
python parse_attack_graph_v30.py --tm7 "../in/Automotive DFD_B.tm7" --type remote --target "Door" --boundary "External Vehicle Boundary"  --asset-map "threat_library/asset_to_threats_ver0.2.json" --threat-map "threat_library/threat_to_tactic.json"  --attack-vector-map "threat_library/attack_vector_feasibility.json" --dependency-map "threat_library/dependency.json"  --out "../out/attack_graph_remote_filtered.json" --render-attack-tree --attack-tree-png "../out/attack_tree_remote_filtered.png" --merged-graph-out "../out/merged_attack_graph_remote_filtered" --detection-report "../out/result_report_v2.html" --impact-map "threat_library/impact_map.json"
python parse_attack_graph_v30.py --tm7 "../in/Automotive DFD_B.tm7" --type physical --target "Door" --boundary "External Vehicle Boundary"  --asset-map "threat_library/asset_to_threats_ver0.2.json" --threat-map "threat_library/threat_to_tactic.json"  --attack-vector-map "threat_library/attack_vector_feasibility.json" --dependency-map "threat_library/dependency.json"  --out "../out/attack_graph_physical_filtered.json" --render-attack-tree --attack-tree-png "../out/attack_tree_physical_filtered.png" --merged-graph-out "../out/merged_attack_graph_physical_filtered" --detection-report "../out/result_report_v2.html" --impact-map "threat_library/impact_map.json"
python parse_attack_graph_v30.py --tm7 "../in/Automotive DFD_B.tm7" --type local --target "Door" --boundary "External Vehicle Boundary"  --asset-map "threat_library/asset_to_threats_ver0.2.json" --threat-map "threat_library/threat_to_tactic.json"  --attack-vector-map "threat_library/attack_vector_feasibility.json" --dependency-map "threat_library/dependency.json"  --out "../out/attack_graph_local_filtered.json" --render-attack-tree --attack-tree-png "../out/attack_tree_local_filtered.png" --merged-graph-out "../out/merged_attack_graph_local_filtered" --detection-report "../out/result_report_v2.html" --impact-map "threat_library/impact_map.json"
python parse_attack_graph_v30.py --tm7 "../in/Automotive DFD_B.tm7" --type adjacent --target "Door" --boundary "External Vehicle Boundary"  --asset-map "threat_library/asset_to_threats_ver0.2.json" --threat-map "threat_library/threat_to_tactic.json"  --attack-vector-map "threat_library/attack_vector_feasibility.json" --dependency-map "threat_library/dependency.json"  --out "../out/attack_graph_adjacent_filtered.json" --render-attack-tree --attack-tree-png "../out/attack_tree_adjacent_filtered.png" --merged-graph-out "../out/merged_attack_graph_adjacent_filtered" --detection-report "../out/result_report_v2.html" --impact-map "threat_library/impact_map.json"
