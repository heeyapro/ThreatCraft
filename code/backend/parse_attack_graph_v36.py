...
def generate_html_report(
    ...
    # If out_path is missing or invalid, use empty dict (AI data alone is sufficient)
    try:
    ...
    if gemini_vehicle_review or gemini_functional:
        ai_badge = "<span style='background:#8b5cf6;color:white;padding:2px 10px;border-radius:4px;font-size:11px;margin-left:8px'>AI Analysis Included</span>"
...
<body>
    ...
    <p class=\"note\">This report is structured in line with the illustrative workflow of ISO/SAE 21434 Annex H and uses the example CAL mapping from Annex E Table E.1 for concept-phase reporting. Sections 10–11 are AI-generated (Gemini) and require expert review.</p>
...
def main():
    ...
    # Backend does not call AI directly. Frontend generates gemini_analysis.json,
    # then calls this backend with --regenerate-report to embed AI results in report.
    gemini_vehicle_review = None
    gemini_functional = None
    ...
    if not gemini_vehicle_review and not gemini_functional:
        ...
        for _d in _search_dirs:
            ...
            if _dk not in _seen_dirs:
                _seen_dirs.add(_dk); _unique_dirs.append(_d)
    ...
    if args.detection_report:
        try:
            # Resolve attack_graph_path: prefer PNG from merged graph render
            _ag_path = ""
            if rendered and Path(str(rendered)).exists():
                _ag_path = str(rendered)
            elif report_preview_graph and Path(str(report_preview_graph)).exists():
            ...
            import traceback as _tb
            print(f"[WARN] Report generation error: {_rpt_e}", file=sys.stderr)
