...
# -*- coding: utf-8 -*-
"""
tool_threat_mapper.py  [Code 1]
DFD → 공격 경로 필터 → 자산 속성 매핑 → 위협 + CWE 결과 + 공격 그래프
"""
...
DEFAULT_BACKEND    = (_SCRIPT_DIR / "../backend/parse_attack_graph_v36.py").resolve()
DEFAULT_ASSET_MAP  = (_SCRIPT_DIR / "../backend/threat_library/asset_to_threats_ver0.3.json").resolve()
DEFAULT_THREAT_MAP = (_SCRIPT_DIR / "../backend/threat_library/threat_to_tactic_ver0.1.json").resolve()
DEFAULT_AV_MAP     = (_SCRIPT_DIR / "../backend/threat_library/attack_vector_feasibility_ver0.1.json").resolve()
...
HIERARCHY_JSON     = _SCRIPT_DIR / "hierarchy_data.json"
...
def _load_h() -> dict:
    ...
    if _H is None:
        if not HIERARCHY_JSON.exists():
            raise FileNotFoundError(f"hierarchy_data.json 없음: {HIERARCHY_JSON}")
...
# ── 백엔드 실행 ─────────────────────────────────────────────────
# (03.17 메모) LLM API 키 인 llm_api_key 파라미터 추가
def run_path_filter(backend_path,tm7_path,mode,target,boundary,
    ...
    # (03.17 메모) 백엔드 스크립트 실행 시 LLM API키 같이 전달     
    if llm_api_key:
    ...
    if not out_file.exists():
        raise RuntimeError(f"백엔드 실패 (code={proc.returncode}):\n{(proc.stderr or proc.stdout or '')[:600]}")
...
def build_attack_graph_dot(result: dict, mapping: List[dict]) -> str:
    ...
    for g, ph in asset_phase.items():
        ...
        if n_cwe or n_thr:
            label_parts.append(f'<TR><TD ALIGN="LEFT"><FONT POINT-SIZE="8" COLOR="#555">CWE:{n_cwe}  위협:{n_thr}</FONT></TD></TR>')
        if threats_in_graph:
...
class AssetMapDialog(tk.Toplevel):
    def __init__(self, parent, elements, at_data=None):
        ...
        self.geometry("1160x660"); self.minsize(940,460)
        _center(self,1160,660); self.configure(bg="white"); self.grab_set()
    ...
    def _build(self):
        ...
        ch=tk.Frame(self,bg="#2C3E50"); ch.pack(fill="x")
        # Header grid matches _MapRow columns exactly
        _hcols=[("No.",0,3),("Type",1,6),("Name",2,20),("Phase",3,8),
                ("Category",4,12),("Asset Type",5,14),("Asset Kind",6,20),
                ("Property",7,13),("CWE",8,5),("Threats",9,12)]
        for txt,col,w in _hcols:
            tk.Label(ch,text=txt,font=("Arial",9,"bold"),fg="white",
                     bg="#2C3E50",width=w,anchor="center",pady=4
                     ).grid(row=0,column=col,padx=1,sticky="nsew")
        ch.columnconfigure(6,weight=1)
        ch.columnconfigure(7,weight=1)
        
        wrap=tk.Frame(self,bg="white"); wrap.pack(fill="both",expand=True)
        canvas=tk.Canvas(wrap,bg="white",highlightthickness=0)
        sb=ttk.Scrollbar(wrap,orient="vertical",command=canvas.yview)
        self._sf=tk.Frame(canvas,bg="white")
        ...
        for i,el in enumerate(self.elements):
            row=_MapRow(self._sf,i,el,self.at_data); row.frame.pack(fill="x",pady=0); self.rows.append(row)
...
class _MapRow:
    def __init__(self, parent, idx, el, at_data=None):
        ...
        self.v_cat=tk.StringVar(); self.v_type=tk.StringVar()
        self.v_kind=tk.StringVar(); self.v_ncwe=tk.StringVar(value="—")
        ...
        self._build(bg,ph,etype)
        self.v_cat.trace_add("write",self._on_cat)
        self.v_type.trace_add("write",self._on_type)
        self.v_kind.trace_add("write",self._on_kind)

    def _build(self,bg,ph,etype):
        f=self.frame; px=1
        tk.Label(f,text=str(self.idx+1),width=3,bg="#EFEFEF",
                 font=("Arial",9),fg="#888",anchor="center").grid(row=0,column=0,padx=px,pady=2,sticky="nsew")
        tc="#1B5E20" if etype=="node" else "#0D47A1"
        tbg="#E8F5E9" if etype=="node" else "#E3F2FD"
        tk.Label(f,text="NODE" if etype=="node" else "EDGE",
                 width=6,bg=tbg,font=("Arial",8,"bold"),fg=tc,anchor="center",
                 relief="flat").grid(row=0,column=1,padx=px,pady=2,sticky="nsew")
        tk.Label(f,text=self.el.get("name","?")[:40],
                 width=20,bg=bg,font=("Arial",9),fg="#1A1A1A",anchor="w").grid(row=0,column=2,padx=px,pady=2,sticky="nsew")
        ph_clr={"In":"#1B5E20","Through":"#E65100","Out":"#C62828","Entry":"#777"}.get(ph,"#555")
        tk.Label(f,text=ph,width=8,bg=bg,font=("Arial",8,"bold"),
                 fg=ph_clr,anchor="center").grid(row=0,column=3,padx=px,pady=2,sticky="nsew")
        self.cb_cat=ttk.Combobox(f,textvariable=self.v_cat,width=12,state="readonly",font=("Arial",9))
        self.cb_cat["values"]=_list_cats()
        self.cb_cat.grid(row=0,column=4,padx=px,pady=2,sticky="ew")
        self.cb_type=ttk.Combobox(f,textvariable=self.v_type,width=14,state="disabled",font=("Arial",9))
        self.cb_type.grid(row=0,column=5,padx=px,pady=2,sticky="ew")
        self.cb_kind=ttk.Combobox(f,textvariable=self.v_kind,width=20,state="disabled",font=("Arial",9))
        self.cb_kind.grid(row=0,column=6,padx=px,pady=2,sticky="ew")
        self._pbtn=tk.Button(f,text="Select Properties",width=13,font=("Arial",8),
                              relief="flat",bd=1,bg="#E8F0FE",fg="#1A73E8",
                              cursor="hand2",state="disabled",command=self._pick)
        self._pbtn.grid(row=0,column=7,padx=px,pady=2,sticky="ew")
        tk.Label(f,textvariable=self.v_ncwe,width=5,bg="#FFF8F0",
                 font=("Arial",9,"bold"),fg="#E65100",anchor="center").grid(row=0,column=8,padx=px,pady=2,sticky="nsew")
        ...
        self._thr_lbl=tk.Label(f,text=preview or "—",width=12,bg=bg,
                                font=("Arial",8),fg="#1565C0",anchor="w")
        self._thr_lbl.grid(row=0,column=9,padx=px,pady=2,sticky="nsew")
        for col in [6,7]: f.columnconfigure(col,weight=1)

    def _on_cat(self,*_):
        cat=self.v_cat.get(); self.v_type.set(""); self.v_kind.set("")
        self._selected_props=[]; self._upbtn(); self.v_ncwe.set("—")
        self.cb_type["state"]="readonly" if cat else "disabled"
        self.cb_type["values"]=_list_types(cat) if cat else []
        self.cb_kind["state"]="disabled"; self.cb_kind["values"]=[]; self._pbtn["state"]="disabled"
    def _on_type(self,*_):
        cat,typ=self.v_cat.get(),self.v_type.get()
        self.v_kind.set(""); self._selected_props=[]; self._upbtn(); self.v_ncwe.set("—")
        kinds=_list_kinds(cat,typ) if (cat and typ) else []
        self.cb_kind["state"]="readonly" if kinds else "disabled"
        self.cb_kind["values"]=kinds; self._pbtn["state"]="disabled"
    def _on_kind(self,*_):
        cat,typ,knd=self.v_cat.get(),self.v_type.get(),self.v_kind.get()
        self._selected_props=[]; self._upbtn()
        if not (cat and typ and knd): self._pbtn["state"]="disabled"; self.v_ncwe.set("—"); return
        self._pbtn["state"]="normal" if _list_props(cat,typ,knd) else "disabled"
        self._upcwe()
    def _pick(self):
        cat,typ,knd=self.v_cat.get(),self.v_type.get(),self.v_kind.get()
    ...
    def _upbtn(self):
        ...
        if n==0: self._pbtn["text"]="Select Properties"; self._pbtn["bg"]="#E8F0FE"; self._pbtn["fg"]="#1A73E8"
        else: self._pbtn["text"]=f"{n} selected"; self._pbtn["bg"]="#1565C0"; self._pbtn["fg"]="white"
    def _upcwe(self):
        cat,typ,knd=self.v_cat.get(),self.v_type.get(),self.v_kind.get()
    ...
    def to_dict(self):
        ...
        props=self._selected_props or None
        cwes=_get_cwes_merged(cat,typ,knd,props) if (cat and typ and knd) else []
        h_thr=_get_threats_from_hierarchy(cat,typ,knd,props) if (cat and typ and knd) else []
        dfd_thr=_get_dfd_threats(self.el.get("name",""),self.at_data)
        threats=_merge_threats(h_thr,dfd_thr)
        return {"name":self.el.get("name",""),"type":self.el.get("type","node"),
                "phase":self.el.get("phase",""),"asset_guid":self.el.get("asset_guid",""),
                "category":cat,"asset_type":typ,"asset_kind":knd,
                "asset_properties":list(props) if props else [],
                "source":_get_source(cat,typ,knd) if (cat and typ and knd) else None,
                "cwes":cwes,"cwe_count":len(cwes),"threats":threats,"threat_count":len(threats)}
    def clear(self):
        self.v_cat.set(""); self.v_type.set(""); self.v_kind.set("")
        self._selected_props=[]; self._upbtn(); self.v_ncwe.set("—")
        self.cb_type["state"]="disabled"; self.cb_kind["state"]="disabled"; self._pbtn["state"]="disabled"
...
class ResultWindow(tk.Toplevel):
    def __init__(self, parent, mapping, out_json, out_csv,
        ...
        self.title("매핑 결과"); self.geometry("1200x740"); _center(self,1200,740)
    ...
    def _build(self, mapping, out_json, out_csv):
        hdr=tk.Frame(self,bg="#1C2333"); hdr.pack(fill="x")
        tk.Label(hdr,text=f"위협 + CWE 매핑  |  {len(mapping)}개 자산/엣지",
                 font=("Arial",11,"bold"),fg="white",bg="#1C2333",pady=8).pack(side="left",padx=14)
        ...
        tab1=tk.Frame(nb,bg="white"); nb.add(tab1,text="자산 / 엣지")
        cols=("유형","이름","Phase","카테고리","자산종류","속성(수)","소스","CWE","위협")
        tree=ttk.Treeview(tab1,columns=cols,show="headings",height=10)
        for c,w in zip(cols,[55,185,65,110,160,60,75,55,55]):
            tree.heading(c,text=c)
            tree.column(c,width=w,anchor="center" if c in ["유형","Phase","소스","CWE","위협","속성(수)"] else "w")
        ...
        for m in mapping:
            ...
            tree.insert("","end",values=(
                ...
                m["name"][:45],ph,
                m.get("category") or "—",m.get("asset_kind") or "—",
                f"{n_p}개" if n_p else "—",
                m.get("source") or "—",m["cwe_count"],m["threat_count"]),tags=(tag,))

        nb2=ttk.Notebook(tab1); nb2.pack(fill="x",padx=2,pady=4)
        tab_c=tk.Frame(nb2); nb2.add(tab_c,text="CWE")
        dt=ttk.Treeview(tab_c,columns=("ID","취약점명","출처","설명"),show="headings",height=5)
        dt.column("ID",width=85,anchor="center"); dt.column("취약점명",width=255)
        dt.column("출처",width=85,anchor="center"); dt.column("설명",width=450)
        for c in ("ID","취약점명","출처","설명"): dt.heading(c,text=c)
        ...
        tab_t=tk.Frame(nb2); nb2.add(tab_t,text="위협")
        tt2=ttk.Treeview(tab_t,columns=("TID","위협명","Tactic","출처"),show="headings",height=5)
        tt2.column("TID",width=90,anchor="center"); tt2.column("위협명",width=330)
        tt2.column("Tactic",width=160); tt2.column("출처",width=80,anchor="center")
        for c in ("TID","위협명","Tactic","출처"): tt2.heading(c,text=c)
    ...
    def _build_graph_tab(self, tab):
        ...
        self._graph_status.config(text="'PNG 저장/열기' 버튼을 클릭하면 그래프를 렌더링합니다")
    ...
    def _render_graph(self):
        ...
        if ok:
            self._graph_status.config(text=f"PNG 저장: {Path(png_path).name}")
            self._show_png(png_path)
            try: os.startfile(png_path)
            except: pass
        else:
            self._graph_status.config(text="graphviz(dot) 없음 → DOT 파일만 저장됨")
            messagebox.showinfo("안내",
                f"Graphviz가 설치되지 않았습니다.\n"
                f"DOT 파일을 저장했습니다:\n{dot_path}\n\n"
                f"https://graphviz.org 에서 설치 후 다시 시도하세요.")
    ...
    def _show_png(self, png_path):
        try:
            ...
            self._graph_status.config(text="PNG 저장됨 (미리보기: pip install Pillow)")

    def _show_dot(self):
        if not self._ag_result: return
        dot_src=build_attack_graph_dot(self._ag_result, self._mapping)
        win=tk.Toplevel(self); win.title("DOT 소스")
        ...
        tk.Button(win,text="클립보드 복사",font=("Arial",9),relief="flat",
...
def build_result_json(mapping, meta):
    return {"meta":{"generated_at":datetime.now().isoformat(timespec="seconds"),
                    "tool":"tool_threat_mapper v4",**meta},
            "assets":[{"name":m["name"],"element_type":m["type"],"phase":m["phase"],
                        "asset_guid":m.get("asset_guid",""),"category":m["category"],
                        "asset_type":m["asset_type"],"asset_kind":m["asset_kind"],
                        "asset_properties":m.get("asset_properties",[]),
                        "source":m["source"],"cwe_count":m["cwe_count"],
                        "threat_count":m["threat_count"],
                        "cwes":m["cwes"],"threats":m.get("threats",[])}
                       for m in mapping]}

def build_result_csv(mapping):
    buf=io.StringIO(); w=csv.writer(buf)
    w.writerow(["element_type","name","phase","category","asset_type","asset_kind",
                "asset_properties","source","cwe_id","cwe_name","cwe_sources","cwe_desc",
                "threat_id","threat_name","threat_tactic","threat_source"])
    for m in mapping:
        base=[m.get("type",""),m["name"],m["phase"],
              m.get("category") or "",m.get("asset_type") or "",
              m.get("asset_kind") or "","|".join(m.get("asset_properties") or []),
              m.get("source") or ""]
...
class ThreatMapperGUI(tk.Tk):
    ...
    def _build_ui(self):
        hdr=tk.Frame(self,bg="#1C2333"); hdr.pack(fill="x")
        tk.Label(hdr,text="Threat Mapper  —  DFD 기반 자산 속성 매핑 + 위협 / CWE + 공격 그래프",
                 font=("Arial",11,"bold"),fg="white",bg="#1C2333",pady=9).pack(side="left",padx=14)
        inp=tk.LabelFrame(self,text=" 설정 ",font=("Arial",9,"bold"),
        ...
        _row(inp,0,"백엔드 스크립트",self.v_backend,self._br_backend)
        _row(inp,1,"TM7 파일",self.v_tm7,self._br_tm7)
        ...
        for idx,(lbl,var,extra) in enumerate([
            ("Target 자산",self.v_target,None),("Trust Boundary",self.v_boundary,None),
            ("공격 모드",None,"mode"),("Max Depth",self.v_depth,None),
            ("출력 폴더",self.v_out_dir,None),("프로젝트명",self.v_proj,None)]):
        ...
        tk.Button(btn_f,text="▶  실행",bg="#1565C0",fg="white",command=self._run,**bkw).pack(side="left",padx=4)
        tk.Button(btn_f,text="저장",bg="#2E7D32",fg="white",command=self._save,**bkw).pack(side="left",padx=4)
    ...
    def _validate(self):
        ...
        if not b or not Path(b).exists():
            messagebox.showerror("오류",f"백엔드 스크립트를 찾을 수 없습니다:\n{b}"); return False
        if not self.v_tm7.get() or not Path(self.v_tm7.get()).exists():
            messagebox.showerror("오류","TM7 파일을 선택하세요."); return False
        if not self.v_target.get().strip():
            messagebox.showerror("오류","Target 자산명을 입력하세요."); return False
        if not self.v_boundary.get().strip():
            messagebox.showerror("오류","Trust Boundary를 입력하세요."); return False
        return True
    def _run(self):
        if not self._validate(): return
        self.lbl_status.config(text="실행 중…"); self.update()
        ap=self.v_asset.get()
        if ap and Path(ap).exists():
            try:
                with open(ap,encoding="utf-8") as f: self._at_data=json.load(f)
                self._log(f"[OK] asset_to_threats 로드: {Path(ap).name}")
            except Exception: self._at_data=None
        backend_path=Path(self.v_backend.get().strip()).resolve()
        def worker():
            try:
                self.after(0,lambda:self._log(f"[{datetime.now():%H:%M:%S}] 백엔드 실행 시작"))
                ...
                self.after(0,lambda:self._log(f"[OK] 경로 {n_p}개, 노드 {n_n}개"))
                if bundle.get("attack_graph_png"):
                    self.after(0,lambda:self._log(f"[OK] 공격 그래프 PNG: {bundle['attack_graph_png']}"))
                if bundle.get("report_html"):
                    self.after(0,lambda:self._log(f"[OK] 결과보고서 HTML: {bundle['report_html']}"))
                elements=extract_elements(result)
                self.after(0,lambda:self._log(
                    f"[OK] 노드 {sum(1 for e in elements if e['type']=='node')}개  "
                    f"엣지 {sum(1 for e in elements if e['type']=='edge')}개"))
                ...
                self.after(0,lambda:self.lbl_status.config(text="오류"))
        threading.Thread(target=worker,daemon=True).start()
    def _show_mapping(self,elements,result):
        self.lbl_status.config(text="속성 매핑…")
        dlg=AssetMapDialog(self,elements,at_data=self._at_data)
        self.wait_window(dlg)
        if not dlg.confirmed: self.lbl_status.config(text="취소"); return
        ...
        ts=datetime.now().strftime("%Y%m%d_%H%M%S")
        out_json=str(out_dir/f"threat_mapping_{ts}.json"); out_csv=str(out_dir/f"threat_mapping_{ts}.csv")
        ...
        self.lbl_status.config(text=f"완료  {len(mapping)}개 자산")
        ResultWindow(self,mapping,out_json,out_csv,ag_result=self._ag_result)
    def _save(self):
        if not self._mapping_cache: messagebox.showinfo("정보","먼저 실행하세요."); return
