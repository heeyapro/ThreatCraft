...
DFD → Attack Path Filter → Asset Property Mapping → Threat + CWE Results + Attack Graph
"""
...
HIERARCHY_JSON     = _SCRIPT_DIR / "hierarchy_data_ver0.1.json"
...
def _load_h() -> dict:
    ...
    if _H is None:
        if not HIERARCHY_JSON.exists():
            raise FileNotFoundError(f"hierarchy_data_ver0.1.json not found: {HIERARCHY_JSON}")
...
def build_attack_graph_dot(result: dict, mapping: List[dict]) -> str:
    ...
    for g, ph in asset_phase.items():
        ...
        if n_cwe or n_thr:
            label_parts.append(f'<TR><TD ALIGN="LEFT"><FONT POINT-SIZE="8" COLOR="#555">CWE:{n_cwe}  Threats:{n_thr}</FONT></TD></TR>')
        if threats_in_graph:
...
class AssetMapDialog(tk.Toplevel):
    def __init__(self, parent, elements, at_data=None):
        ...
        self.geometry("1300x660"); self.minsize(940,460)
        _center(self,1300,660); self.configure(bg="white"); self.grab_set()
    ...
    def _build(self):
        ...
        _hcols=[("No.",0,3),("Type",1,6),("Name",2,20),("Phase",3,8),
                ("Category",4,12),("Asset Type",5,14),("Asset Kind",6,18),
                ("Property",7,14),("CWE",8,3),("Threats",9,12)]
...
class _MapRow:
    ...
    def _build(self,bg,ph,etype):
        ...
        self._pbtn=tk.Button(f,text="Select Properties",width=14,font=("Arial",8),
...
class ResultWindow(tk.Toplevel):
    def __init__(self, parent, mapping, out_json, out_csv,
        ...
        self.title("Mapping Results"); self.geometry("1200x740"); _center(self,1200,740)
    ...
    def _build(self, mapping, out_json, out_csv):
        ...
        cols=("Type","Name","Phase","Category","Asset Kind","Properties (#)","Source","CWE","Threats")
        tree=ttk.Treeview(tab1,columns=cols,show="headings",height=10)
        for c,w in zip(cols,[55,185,65,110,160,60,75,55,55]):
            tree.heading(c,text=c)
            tree.column(c,width=w,anchor="center" if c in ["Type","Phase","Source","CWE","Threats","Properties (#)"] else "w")
...
class ThreatMapperGUI(tk.Tk):
    ...
    def _build_ui(self):
        hdr=tk.Frame(self,bg="#1C2333"); hdr.pack(fill="x")
        tk.Label(hdr,text="Threat Mapper  —  DFD-Based Asset Property Mapping + Threats / CWE + Attack Graph",
