...
DEFAULT_BACKEND    = (_SCRIPT_DIR / "../../backend/parse_attack_graph_enterprise.py").resolve()
DEFAULT_ASSET_MAP  = (_SCRIPT_DIR / "../../backend/threat_library/enterprise/asset_to_threats_enterprise.json").resolve()
DEFAULT_THREAT_MAP = (_SCRIPT_DIR / "../../backend/threat_library/enterprise/threat_to_tactic_enterprise.json").resolve()
DEFAULT_AV_MAP     = (_SCRIPT_DIR / "../../backend/threat_library/enterprise/attack_vector_feasibility_enterprise.json").resolve()
DEFAULT_DEP_MAP    = (_SCRIPT_DIR / "../../backend/threat_library/enterprise/dependency_enterprise.json").resolve()
DEFAULT_IMPACT_MAP = (_SCRIPT_DIR / "../../backend/threat_library/enterprise/impact_map_enterprise.json").resolve()
DEFAULT_OUT_DIR    = (_SCRIPT_DIR / "../../out").resolve()
HIERARCHY_JSON     = _SCRIPT_DIR / "hierarchy_data_enterprise.json"
...
def _load_h() -> dict:
    ...
    if _H is None:
        if not HIERARCHY_JSON.exists():
            raise FileNotFoundError(f"hierarchy_data_enterprise.json not found: {HIERARCHY_JSON}")
...
def _compat_filtered_pred_possible(
    ...
    for pid in (predecessor_ids or []):
        pred_cls = id_to_class.get(str(pid), "")
        raw = _get_asset_threats_set(pid, at_data)
        if pred_cls and compat:
    
            result |= {t for t in raw if pred_cls in compat.get(t, [pred_cls])}
        else:
            result |= raw
    return result
...
def _asset_id_from_name(asset_name: str, at_data: Optional[dict]) -> Optional[str]:
    """Look up an Enterprise asset_id such as A0003 from asset_to_threats_enterprise.json."""
...
def _filter_threats(
    ...
    if chains:
        pred_possible: Set[str] = _compat_filtered_pred_possible(
            predecessor_ids, at_data, id_to_class, compat
        )
...
def build_result_json(mapping, meta):
    return {"meta":{"generated_at":datetime.now().isoformat(timespec="seconds"),
                    "tool":"tool_threat_mapper_enterprise (multi-category + Enterprise dependency filter)",**meta},
...
class ThreatMapperGUI(tk.Tk):
    ...
    def _build_ui(self):
        hdr=tk.Frame(self,bg="#1C2333"); hdr.pack(fill="x")
        tk.Label(hdr,text="Enterprise Threat Mapper  —  DFD-Based Asset Property Mapping + Threats / CWE + Attack Graph",
    ...
    def _show_mapping(self,elements,result):
        ...
        ts=datetime.now().strftime("%Y%m%d_%H%M%S")
        out_json=str(out_dir/f"enterprise_threat_mapping_{ts}.json"); out_csv=str(out_dir/f"enterprise_threat_mapping_{ts}.csv")
